...
__global__ void spFeatGather2d_cuda_forward_kernel(
    ...
    int depth, int height, int width, int ignore_idx_value) {
    // indexing
    ...
    const int w = d % width;
    if (h < height) {
        const int spix_idx = static_cast<int>(init_spIndx[n][0][h][w]);
        if (spix_idx != ignore_idx_value) {
            for (int k = 0; k < depth; k++) {
                atomicAdd(&spFeat[n][k][spix_idx], pFeat[n][k][h][w]);
            }
            atomicAdd(&spSize[n][spix_idx], 1.0);
        }
    }
}

template <typename scalar_t>
__global__ void spFeatGather2d_normalize_cuda_forward_kernel(
    torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> spFeat,
    const torch::PackedTensorAccessor<scalar_t,2,torch::RestrictPtrTraits,size_t> spSize,
    int depth, int K, int ignore_feature_value) {
    // indexing
    const int n = blockIdx.y;
    const int spix_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (spix_idx < K) {
        bool zeroSize = (spSize[n][spix_idx] < 0.001);
        for (int k = 0; k < depth; k++) {
            if (zeroSize) {
...
std::vector<torch::Tensor> spFeatGather2d_cuda_forward(
    ...
    AT_DISPATCH_FLOATING_TYPES(pFeat.type(), "spFeatGather2d_forward_cuda", ([&] {
        spFeatGather2d_cuda_forward_kernel<scalar_t><<<blocks, threads>>>(
            ...
            depth, height, width, ignore_idx_value);
    }));
    const dim3 blocks2((K + threads - 1) / threads, batch_size);
    AT_DISPATCH_FLOATING_TYPES(pFeat.type(), "spFeatGather2d_normalize_forward_cuda", ([&] {
        spFeatGather2d_normalize_cuda_forward_kernel<scalar_t><<<blocks2, threads>>>(
            spFeat.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
            spSize.packed_accessor<scalar_t,2,torch::RestrictPtrTraits,size_t>(),
