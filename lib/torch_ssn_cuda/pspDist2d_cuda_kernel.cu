...
__global__ void pspDist2d_cuda_forward_kernel(
    ...
    torch::PackedTensorAccessor<scalar_t,4,torch::RestrictPtrTraits,size_t> sqdist,
    ...
    if (c < 9) {
        ...
        if (invalid_spixel) {
          ...
          for (int k = 0; k < depth; k++) {
            sq_dist += pow(pFeat[n][k][h][w] - spFeat[n][k][spix_idx], 2);
          }
        }
        sqdist[n][c][h][w] = sq_dist;
    }
}

template <typename scalar_t>
__global__ void pspDist2d_cuda_backward_kernel(
    const torch::PackedTensorAccessor<scalar_t,4,torch::RestrictPtrTraits,size_t> grad_sqdist,
    ...
    if (c < 9) {
        ...
        if ( !invalid_spixel ) {
            for (int k = 0; k < depth; k++) {
                scalar_t _grad_pFeat = grad_sqdist[n][c][h][w] * 2 * (pFeat[n][k][h][w] - spFeat[n][k][spix_idx]);
                atomicAdd(&grad_pFeat[n][k][h][w], _grad_pFeat);
...
torch::Tensor pspDist2d_cuda_forward(
    ...
    const int K = Kh * Kw;
    auto sqdist = torch::zeros({batch_size, 9, height, width},
        torch::TensorOptions().dtype(pFeat.dtype()).device(pFeat.device()).requires_grad(true));  // B 9 H W
    // launch kernel
    const int threads = 1024;
    const dim3 blocks((9 * height * width + threads - 1) / threads, batch_size);
    AT_DISPATCH_FLOATING_TYPES(pFeat.type(), "pspDist2d_forward_cuda", ([&] {
        pspDist2d_cuda_forward_kernel<scalar_t><<<blocks, threads>>>(
            ...
            sqdist.packed_accessor<scalar_t,4,torch::RestrictPtrTraits,size_t>(),
...
std::vector<torch::Tensor> pspDist2d_cuda_backward(
    const torch::Tensor grad_sqdist,  // B 9 H W
    const torch::Tensor pFeat,  // B C H W
    const torch::Tensor spFeat,  // B C K
    const torch::Tensor init_spIndx,  // B 1 H W
    const int Kh,
    ...
    const int threads = 1024;
    const dim3 blocks((9 * height * width + threads - 1) / threads, batch_size);
    AT_DISPATCH_FLOATING_TYPES(spFeat.type(), "pspDist2d_backward_cuda", ([&] {
        pspDist2d_cuda_backward_kernel<scalar_t><<<blocks, threads>>>(
            grad_sqdist.packed_accessor<scalar_t,4,torch::RestrictPtrTraits,size_t>(),
