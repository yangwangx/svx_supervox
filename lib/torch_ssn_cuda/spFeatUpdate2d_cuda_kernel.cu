...
__global__ void spFeatUpdate2d_cuda_forward_kernel(
    const torch::PackedTensorAccessor<scalar_t,4,torch::RestrictPtrTraits,size_t> pFeat,
    const torch::PackedTensorAccessor<scalar_t,4,torch::RestrictPtrTraits,size_t> assoc,
    ...
    if (c < 9) {
        ...
        if ( !invalid_spixel ) {
            ...
            atomicAdd(&spWght[n][spix_idx], assoc[n][c][h][w]);
        }
    }
}

template <typename scalar_t>
__global__ void spFeatUpdate2d_normalize_cuda_forward_kernel(
    torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> spFeat,
...
template <typename scalar_t>
__global__ void spFeatUpdate2d_cuda_backward_kernel(
    const torch::PackedTensorAccessor<scalar_t,3,torch::RestrictPtrTraits,size_t> grad_spFeat,
    torch::PackedTensorAccessor<scalar_t,4,torch::RestrictPtrTraits,size_t> grad_pFeat,
    torch::PackedTensorAccessor<scalar_t,4,torch::RestrictPtrTraits,size_t> grad_assoc,
    ...
    if (c < 9) {
        ...
        if ( !invalid_spixel ) {
            ...
            for (int k = 0; k < depth; k++) {
                if ( nonzeroWght ) {
                    atomicAdd(&grad_pFeat[n][k][h][w], 
                        grad_spFeat[n][k][spix_idx] * assoc[n][c][h][w] / spWght[n][spix_idx]);
                    atomicAdd(&grad_assoc[n][c][h][w],
                        grad_spFeat[n][k][spix_idx] * (pFeat[n][k][h][w] - spFeat[n][k][spix_idx]) / spWght[n][spix_idx]);
                }
            }
        }
    }
...
std::vector<torch::Tensor> spFeatUpdate2d_cuda_forward(
    const torch::Tensor pFeat,  // B C H W
    const torch::Tensor assoc,  // B 9 H W
    ...
    const auto height = pFeat.size(2);
    ...
    const dim3 blocks((9 * height * width + threads - 1) / threads, batch_size);
    AT_DISPATCH_FLOATING_TYPES(pFeat.type(), "spFeatUpdate2d_forward_cuda", ([&] {
        spFeatUpdate2d_cuda_forward_kernel<scalar_t><<<blocks, threads>>>(
            pFeat.packed_accessor<scalar_t,4,torch::RestrictPtrTraits,size_t>(),
            assoc.packed_accessor<scalar_t,4,torch::RestrictPtrTraits,size_t>(),
            ...
            depth, height, width, Kh, Kw, K);
    }));
    const dim3 blocks2((K + threads - 1) / threads, batch_size);
    AT_DISPATCH_FLOATING_TYPES(pFeat.type(), "spFeatUpdate2d_normalize_forward_cuda", ([&] {
        spFeatUpdate2d_normalize_cuda_forward_kernel<scalar_t><<<blocks2, threads>>>(
            spFeat.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
            spWght.packed_accessor<scalar_t,2,torch::RestrictPtrTraits,size_t>(),
            depth, K);
    }));
...
std::vector<torch::Tensor> spFeatUpdate2d_cuda_backward(
    const torch::Tensor grad_spFeat,  // B C 1 K
    ...
    const int threads = 1024;
    const dim3 blocks((9 * height * width + threads - 1) / threads, batch_size);
    AT_DISPATCH_FLOATING_TYPES(spFeat.type(), "spFeatUpdate2d_backward_cuda", ([&] {
        spFeatUpdate2d_cuda_backward_kernel<scalar_t><<<blocks, threads>>>(
            ...
            grad_assoc.packed_accessor<scalar_t,4,torch::RestrictPtrTraits,size_t>(),
            spFeat.packed_accessor<scalar_t,3,torch::RestrictPtrTraits,size_t>(),
            spWght.packed_accessor<scalar_t,2,torch::RestrictPtrTraits,size_t>(),
            pFeat.packed_accessor<scalar_t,4,torch::RestrictPtrTraits,size_t>(),
            assoc.packed_accessor<scalar_t,4,torch::RestrictPtrTraits,size_t>(),
